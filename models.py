...
class User(Base):
    ...
    bio: Mapped[str | None] = mapped_column(Text(500), nullable=True)
...
class Item(Base):
    ...
    title: Mapped[str] = mapped_column(Text(500), nullable=False, index=True)
    content: Mapped[str] = mapped_column(Text(10000), nullable=False)
    ...
    tags: Mapped[list[Tag]] = relationship(secondary="itemtags", back_populates="items")
...
class Tag(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(120), nullable=False, index=True, unique=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC), nullable=False
    )
    ...
    items: Mapped[list[Item]] = relationship(
        secondary="itemtags", back_populates="tags"
    )
...
class Itemtag(Base):
    ...
    item_id: Mapped[int] = mapped_column(
        ForeignKey("items.id"), nullable=False, index=True, primary_key=True
    )
    tag_id: Mapped[int] = mapped_column(
        ForeignKey("tags.id"), nullable=False, index=True, primary_key=True
    )
...
class Follow(Base):
    ...
    # following_id
    follows_id: Mapped[int] = mapped_column(
        ForeignKey("users.id"), primary_key=True, nullable=False, index=True
    )
    ...
    following_user: Mapped[User] = relationship(
        foreign_keys=[follows_id], back_populates="followers"
    )
...
class Like(Base):
    ...
    item_id: Mapped[int] = mapped_column(
        ForeignKey("items.id"), primary_key=True, nullable=False, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC), nullable=False
    )
    ...
    item: Mapped[Item] = relationship(back_populates="likes")
